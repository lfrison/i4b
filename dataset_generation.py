...
def generate_heatcurve_control_dataset(config: dict = DEMO_CONFIG, print_progress: bool = True) -> pd.DataFrame:
    '''
    ...
    # Run simulation
    num_settings = len(config['setting_changes']) + 1
    state = None
    steps_simulated = 0
    switch_times = []
    setting = config['base_setting']
    for i_setting in range(num_settings):
        ...
        for i_sim_step in range(len(df_disturbances)):
            ...
            # Save results
            df_results['T_hp_sup'][i_total_step] = t_sup
            for k in r['state'].keys():
                df_results[k][i_total_step+1] = r['state'][k]
            for k in r['cost'].keys():
                df_results[k][i_total_step+1] = r['cost'][k]

        # Set fault labels
        df_results['Fault'][steps_simulated : (steps_simulated + len(df_disturbances))] = int(setting['faulty'])
